...
def launch(
    ...
    # set config
    assert isinstance(config, (Config, str, Path, dict)), f"expected argument config to be Config, str or Path, but got {type(config)}"
...
def launch_from_slurm(config: Union[str, Path, Config, Dict], host: str, port: int, backend: str = "nccl", seed: int = 1024, verbose: bool = True):
    ...
    launch(config=config, rank=rank, world_size=world_size, host=host, port=port, backend=backend, seed=seed, verbose=verbose)


def launch_from_openmpi(config: Union[str, Path, Config, Dict], host: str, port: int, backend: str = "nccl", seed: int = 1024, verbose: bool = True):
    ...
    launch(config=config, local_rank=local_rank, rank=rank, world_size=world_size, host=host, port=port, backend=backend, seed=seed, verbose=verbose)


def launch_from_torch(config: Union[str, Path, Config, Dict], backend: str = "nccl", seed: int = 1024, verbose: bool = True):
...
def initialize(
    ...
    if use_zero:
        ...
        model, optimizer = convert_to_zero_v2(model, optimizer, model_config=model_config, optimizer_config=optimizer_config)
    ...
    if fp16_cfg is not None and fp16_cfg.mode is not None and use_zero:
        raise ConfigException("It is not allowed to set fp16 and zero configuration in your config file at the same time")
    ...
    if fp16_cfg is not None and fp16_cfg.mode is not None:
        ...
        model, optimizer, criterion = convert_to_amp(model=model, optimizer=optimizer, criterion=criterion, mode=amp_mode, amp_config=cfg_)
    ...
    if gradient_handler_cfg is None:
        ...
        if isinstance(optimizer, ShardedOptimizerV2):
            ...
            if verbose:
                logger.info(
                    "Training with zero is detected, ZeROGradientHandler is automatically " "added even though not specified in the configuration",
                    ranks=[0],
                )
            ...
            model = DDP(model, process_group=gpc.get_group(ParallelMode.SEQUENCE_DP), device_ids=[torch.cuda.current_device()], **torch_ddp_cfg)
            if verbose:
                logger.info("Model is using torch.nn.parallel.DistributedDataParallel for Sequence Parallelism", ranks=[0])
        elif is_using_ddp() and not is_using_pp() and amp_mode != AMP_TYPE.NAIVE:
            model = DDP(model, process_group=gpc.get_group(ParallelMode.DATA), device_ids=[torch.cuda.current_device()], **torch_ddp_cfg)
            if verbose:
        ...
        if not isinstance(gradient_handler_cfg, list):
            raise ConfigException(f"expected gradient_handler in the configuration file to be a list but got {type(gradient_handler_cfg)}")
    ...
    if is_using_pp():
        ...
        if use_interleaved:
            ...
            schedule = InterleavedPipelineSchedule(
                gpc.config.NUM_MICRO_BATCHES, gpc.config.model.num_chunks, tensor_shape=tensor_shape, scatter_gather_tensors=scatter_gather
            )
        else:
            schedule = PipelineSchedule(gpc.config.NUM_MICRO_BATCHES, tensor_shape=tensor_shape, scatter_gather_tensors=scatter_gather)
    else:
