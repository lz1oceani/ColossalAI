...
class ParallelContext(metaclass=SingletonMeta):
    ...
    def _set_parallel_size_from_config(self, config: dict, key: str, attr_name: str):
        if key in config:
            ...
            if isinstance(ele, int):
                ...
                raise NotImplementedError(f'{"Parallel configuration does not support this kind of argument, please use int or dict"}')
    ...
    def init_parallel_groups(self):
        ...
        for initializer_cfg in pg_init:
            ...
            initializer = DIST_GROUP_INITIALIZER.get_module(initializer_type)(
                rank, world_size, self.config, self.data_parallel_size, self.pipeline_parallel_size, self.tensor_parallel_size, **cfg
            )
    ...
    def set_seed(self, seed: int):
        ...
        if torch.cuda.is_available():
            ...
            if self._verbose:
                self._logger.info(f"initialized seed on rank {global_rank}, " f"numpy: {seed}, python random: {seed}, pytorch: {seed}", ranks=[0])
                self._logger.info("WARNING: CUDA is not available, thus CUDA RNG cannot be used to track CUDA random number states", ranks=[0])
